...
export const QuizView: React.FC<{
	...
	if (!item)
		return (
			<div className="p-10 text-center text-slate-400">Loading Session...</div>
		);
					...
					{sessionQueue.map((_, idx) => (
						<div
							...
							className={`h-1.5 w-4 rounded-full transition-colors ${
								idx === currentIndex
									? "bg-gold"
									: results[sessionQueue[idx]] === "correct"
										? "bg-green-400"
										: results[sessionQueue[idx]] === "wrong"
											? "bg-red-300"
											: "bg-stone-200"
							}`}
						/>
						...
						{!isFlipped ? (
							...
							<div className="flex-grow flex flex-col animate-in fade-in slide-in-from-bottom-2 duration-300">
								<div className="flex items-start gap-4 mb-6 border-b border-stone-100 pb-4 pr-12">
									<span className="flex-shrink-0 w-8 h-8 bg-stone-100 rounded-full flex items-center justify-center text-xs font-bold text-stone-500">
										Q
									</span>
								...
								<div className="prose prose-slate max-w-none">
									...
									<FormattedAnswer
										text={item.A}
										onFootnoteClick={(i) =>
											setActiveFootnote((prev) => (prev === i ? null : i))
										}
