...
export const BrowseView: React.FC<{
  ...
  const filteredData = useMemo(() => {
    ...
    if (showBookmarkedOnly) {
      result = result.filter(({ originalId }) => bookmarks.includes(originalId));
    }

    // Filter by search query
    if (q) {
      result = result.filter(({ item, originalId }) => 
    ...
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-4 pb-24">
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <button 
          ...
          `}
        >
          {showBookmarkedOnly ? <Star size={14} fill="currentColor" /> : <Star size={14} />}
      ...
      {filteredData.length === 0 ? (
        ...
        filteredData.map(({ item, originalId }) => (
          <Card 
            ...
            onToggle={() => setExpandedId(expandedId === originalId ? null : originalId)}
